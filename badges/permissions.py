...
class IsProfessorOrEstudanteForView(BasePermission):
    message = 'Apenas professores e estudantes podem visualizar badges.'

    def has_permission(self, request, view):
        if request.method in ['GET', 'HEAD', 'OPTIONS']:
            return bool(request.user and request.user.is_authenticated and request.user.role in [Role.PROFESSOR, Role.ESTUDANTE])
        return True
